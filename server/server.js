...
app.post("/insert-user-detail", (req, res) => {
  ...
  try {
    let userInsert = mysql.createConnection({
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
        [req.body.phone, req.body.password, req.body.first_name, req.body.last_name, req.body.email, req.body.address_line1, req.body.address_line2, req.body.city, req.body.state, req.body.zipcode, req.body.role_id],
        (error, results) => {
          if (error)
            return res.json({ error: error });
          else
